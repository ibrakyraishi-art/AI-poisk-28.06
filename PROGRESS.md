...
## 2026-07-07 — Сверка с изначальным планом + 3 недостающие фичи

Пользователь попросил вернуться к `design.md` и достроить три вещи. Сверка показала:
- **#1 «Сменные мозги» (LLM на каждого агента)** — было в плане (Stage 6: «dropdown
  для каждого агента»), но во фронте отдавалась одна модель на всех. → достроено.
- **#2 Детальная визуализация** — в схеме были `progress_pct`/`progress_msg`, но runner
  их не писал, а AgentPipeline анимировал по таймеру. → сделан реальный прогресс.
- **#3 Подтверждение плана человеком** — в письменном плане НЕТ (retry там «без
  вмешательства пользователя»), но пользователь помнил договорённость. Добавлено как
  новая фича (по согласованию, чекбокс по умолчанию включён).

### #1 Per-agent LLM (коммит `d2a247a`)

- `AnalyzeRequest.agent_models: {agent → model}`; роут валидирует.
- `runner._pick_model_for_agent()` — на каждого агента: явный выбор → общая модель →
  умный дефолт; проверка провайдер-ключа с fail-fast.
- Форма: «Модель для всех агентов» + сворачиваемый блок «⚙ Модель для каждого агента»
  с дропдауном на агента и пресетами (Эконом/Баланс/Максимум).

### #2 Реальный прогресс агентов (коммит `2e12ed2`)

- `runner`: колбэк на каждую из 6 задач → по завершении пишет `progress_pct` +
  `progress_msg` (JSON `{done:[...], note}`). Best-effort, не ломает прогон.
- `AgentPipeline`: читает реальные `progress_pct/progress_msg` — отмечает именно
  завершённых агентов, активного вычисляет, показывает реальную заметку и прогресс-бар.

### #3 Подтверждение плана (коммит `463ce19`)

- Миграция: `analysis_runs.plan_json jsonb`.
- Двухфазный runner: `confirm_plan=true` → **фаза 1** (только Business Context ~1 мин →
  предлагает категорию/аудиторию/конкурентов, статус `awaiting_approval`, план в
  `plan_json`, конкуренты в `competitors`). Юзер правит список → **фаза 2**
  (`POST /analyze/{id}/approve` → полный пайплайн, запиненный на подтверждённых
  конкурентов через `create_competitor_finder_task(competitors=...)`).
- Фронт: чекбокс «Сначала показать план» (по умолчанию вкл), компонент `PlanReview`
  (подход + рыночный контекст + редактируемые чипы конкурентов), состояния
  `planning`/`awaiting_approval` на странице результатов.
- Новые статусы прогона: `planning`, `awaiting_approval` (в дополнение к
  running/completed/failed).

### «Живые роботы» в пайплайне (коммит `f3a478c`)

По фидбэку со скриншота прод-прогона: чек-лист скучный, хочется видеть «как роботы
работают». `AgentPipeline` теперь:
- активный агент покачивается (bob) со спиннером-шестерёнкой и **пузырём речи**,
  где ротацией (3.5 с) идут его реальные под-шаги («Собираю отзывы из App Store…»,
  «Строю SWOT-анализ…») с печатающими точками;
- по коннекторам к активному этапу бегут светящиеся «пакеты данных»;
- готовые — зелёная ✓ и чип «готово», спящие — 💤 «ожидает», менеджер — «координирует»;
- в шапке «N из 6 готово» + таймер.
Честность: отметки завершения — реальные (progress_msg из бэкенда); фразы в пузыре —
типовые шаги задачи агента, ротация на клиенте.

### Сцена «команда роботов» (коммит `3edaf55`)

По запросу «чтобы роботы прям взаимодействовали»: вертикальный список заменён на
SVG-карту команды (дизайн выработан панелью из 3 агентов: моушн/ясность/мобильность):
- менеджер сверху, узел «Общая память» в центре, 6 роботов кольцом;
- единый такт 4.2 с: менеджер «говорит» → пакет-задача летит к активному агенту
  (с хвостом кометы) → агент вздрагивает → пакет данных летит агент→память (зелёный)
  или память→агент (голубой — аналитик и писатель читают память) → память «глотает»;
- готовые: зелёное кольцо + ✓; спящие: приглушены + 💤; активный: двойное пульс-кольцо,
  бегущий пунктир спицы, прожектор; на спицах готовых — редкие ambient-пакеты;
- лента переговоров (новое сверху): «🧭 Менеджер → ⭐ Отзывы: „Собери отзывы…“»,
  живая строка с печатающими точками, фразы в такте 4.2 с от часов (без дрейфа);
- мобильные/a11y: подписи скрыты ≤640px, non-scaling strokes, без backdrop-blur под
  анимацией, role=img/progressbar, sr-only status, prefers-reduced-motion гасит декор.

### ⚠️ Найдено при этом: Render не автодеплоит

Прогон 14:48 завис так же, как раньше (0 embeddings, progress_msg NULL → на Render
старый код без умного дефолта и прогресса). Авто-деплой у сервиса выключен; деплой
случался только вручную. Действие: Render → Settings → Build & Deploy → Auto-Deploy On
+ Manual Deploy latest. Зависший прогон помечен failed вручную.

---

## 2026-07-09 — Keep-alive для Render (закрыта открытая задача #4)

«Вечная загрузка» при запуске анализа = холодный старт free-tier Render (спит после
~15 мин простоя, просыпается 50+ сек — запрос из браузера висит).

**Фикс на стороне Supabase (бэкенд-код не менялся):**
- включено расширение `pg_net` (async HTTP из Postgres);
- pg_cron job `keep-render-awake`: каждые 10 минут `net.http_get(.../health)` —
  Render больше не засыпает (750 бесплатных часов/мес хватает на 24/7 один сервис);
- проверка из базы: `/health` → 200 `{"status":"ok","auth":"es256+hs256"}` —
  подтверждено, что на Render живёт свежая сборка (JWKS-маркер на месте).

---

## 2026-07-09 — 🔴 Найден и убит корень ВСЕХ зависонов: torch OOM (коммит `2d1d4f4`)

Диагностика в два слоя. Локальный Claude (браузер): POST /analyze иногда получает
**503 без CORS-заголовков** → браузер видит «Failed to fetch»; прямые запросы с
токеном проходят (200 + run_id). База: два свежих прогона на НОВОМ бэкенде — по
8 минут «running», progress_msg NULL (а новый код пишет маркер в первые секунды),
0 embeddings. Значит поток умирает ДО первой записи — тихо.

**Корень:** `memory/vector_store.py` импортировал `sentence_transformers` (→ torch)
на уровне модуля. Импорт агентов внутри `_run_crew` тянул torch → сотни МБ на
импорте → **OOM-kill на 512MB free-tier Render** → процесс перезапускается,
поток умирает без записи ошибки, `/health` при этом отвечает (новый процесс).
Это объясняет ВСЕ прошлые «вечные running» с 0 embeddings.

**Фикс:**
- `vector_store` → **fastembed** (ONNX, без torch, та же модель all-MiniLM-L6-v2,
  те же 384-dim), ленивый импорт внутри `_embed_texts()`; при недоступности движка
  память деградирует в no-op (save пропускается, search → [], насыщение False) —
  пайплайн работает дальше через context=.
- `requirements.txt`: sentence-transformers → fastembed (сборка Render сильно легче).
- Усиление runner: тяжёлые импорты и загрузка BYOK-ключей внутри try; первой
  строкой — маркер «Готовлю окружение…» в progress_msg (любая смерть теперь видна).
- Фронт `api.ts`: `fetchWithRetry` (3 попытки, backoff) на startAnalysis/approveAnalysis —
  разовые 503 Render авто-восстанавливаются.
- Два зомби-прогона помечены failed с пояснением.

---

## 2026-07-09 — Сводка состояния проекта (читать при старте новой сессии)

### Поправка к записи «Render не автодеплоит» (2026-07-07)

Events на Render показал: **Auto-Deploy был включён всё время** («On Commit») и
деплоил каждый коммит. Причина эпизода со «старым бэкендом»: сборки с torch шли
по 10+ минут и вставали в очередь (политика Overlapping = Wait), поэтому прогон,
запущенный в 14:48, попал на инстанс, который ещё не успел обновиться. После
перехода на fastembed сборки лёгкие — проблема исчезает сама.

### Что сейчас работает (проверено)

- ✅ Деплой: Vercel (фронт) и Render (бэк) автоматически с каждого пуша в main.
- ✅ CORS (regex `*.vercel.app`) и JWT (HS256 + ES256/JWKS) — вход в API рабочий;
  проверено внешним POST: 200/422 с корректным ACAO (~230–550 мс).
- ✅ Keep-alive: pg_cron `keep-render-awake` пингует /health каждые 10 мин —
  Render не засыпает; `/health` отдаёт маркер сборки `{"auth":"es256+hs256"}`.
- ✅ BYOK-ключи пользователя: Anthropic + Serper + NewsAPI заданы (Google нет —
  дефолт автоматически падает на Claude Haiku).
- ✅ UI: тёмная тема, русский, сцена «команда роботов», план-подтверждение,
  выбор модели на агента, реальный прогресс, авторетрай сетевых сбоев.

### Что ЕЩЁ НЕ проверено (главное на следующую сессию)

- ⏳ **Ни один анализ ещё не прошёл до конца.** Все прошлые падали на torch OOM.
  После деплоя fastembed-сборки (коммит `2d1d4f4`) нужен первый сквозной прогон:
  план → подтверждение → 7 агентов → отчёт (+ .docx). Следить в БД:
  `progress_msg` должен пройти «Готовлю окружение…» → «N — готово» → completed.
- ⏳ Влезает ли fastembed в 512MB вместе с CrewAI на реальном прогоне
  (страховка: при OOM эмбеддингов память станет no-op, пайплайн продолжит).

### Открытые задачи (актуализировано)

| # | Задача | Статус |
|---|--------|--------|
| 1 | Реальный по-агентный прогресс | ✅ Сделано (progress_msg + сцена) |
| 2 | Реальный скрейпер отзывов App Store (убран из-за конфликта deps) | Открыто; Google Play скрейпер стоит, iOS — mock |
| 3 | Сквозной тест на проде до отчёта | ⏳ Блокер снят (OOM), ждёт первого прогона |
| 4 | Render засыпает | ✅ Закрыто (pg_cron keep-alive) |
| 5 | Дубли-прогоны при ретрае POST (редкий кейс: сервер принял, ответ оборвался) | Низкий приоритет |
| 6 | Идея пользователя «окно атрибуции для iOS» — не расшифрована | Уточнить у пользователя |
